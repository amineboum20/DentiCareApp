...
export async function POST(req: Request) {
  ...
  const [practiceRes, teethRes, dossiersRes, visitesRes, facturesRes, ordonnancesRes, planRes] = await Promise.all([
    ...
    db.from("tooth_plan").select("tooth").eq("patient_id", pid).eq("status", "planned"),
  ]);
  ...
  return NextResponse.json({
    ...
    teeth: teethRes.data ?? [],
    plannedTeeth: [...new Set((planRes.data ?? []).map((p: { tooth: string }) => p.tooth))],
