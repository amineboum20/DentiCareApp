...
export const MODULES: TestModule[] = [
  ...
  },
  {
    id: "subscriptions", icon: "💳", title: "Abonnements", tests: [
      ...
      { id: "TC-080", title: "Supprimer définitivement", steps: ["Approbations → « Supprimer… » sur un cabinet de test (ou Abonnements → fiche)", "Lire le résumé de la zone dangereuse, taper un nom erroné puis le nom exact", "Se réinscrire avec l'email du propriétaire"], expected: "Le résumé liste ce qui partira (patients, visites, factures, ordonnances, membres…) ; bouton grisé tant que le nom n'est pas exact ; après suppression : bandeau vert, plus rien dans l'admin, et l'email est réutilisable" },
      { id: "TC-081", title: "Archive avant suppression", steps: ["Après TC-080 : Admin → 🗄️ Archives", "« 🗜️ Télécharger (ZIP) » sur l'archive"], expected: "L'archive apparaît (nom, date, auteur, résumé) ; le ZIP contient index.json, data.json (toutes les lignes + comptes des membres) et files/ (logo, photos, pièces jointes)" },
    ],
  },
  {
