...
export default function SupportThreadActions({ ticketId, status }: { ticketId: string; status: string }) {
  ...
  async function sendReply(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/support/${ticketId}/reply`, { method: "POST", body: fd });
  ...
  return (
    <div className="mt-6 border-t border-zinc-200 dark:border-zinc-800 pt-6">
      {status === "closed" && (
        <div className="mb-4 flex items-center justify-between gap-3 rounded-lg bg-zinc-100 dark:bg-zinc-800 px-4 py-3 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <button onClick={() => setStatus("reopen")} disabled={busy}
            className="shrink-0 text-teal-600 font-medium hover:underline disabled:opacity-60">{t("reopen")}</button>
        </div>
      )}
      ...
      <form onSubmit={sendReply} className="flex flex-col gap-3">
        <textarea placeholder={t("replyPlaceholder")} value={message} rows={4} onChange={e => setMessage(e.target.value)} required
