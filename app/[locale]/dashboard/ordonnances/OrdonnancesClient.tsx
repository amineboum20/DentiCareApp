...
export default function OrdonnancesClient({ initialOrdonnances, patients }: Props) {
  ...
  async function handleSave() {
    if (!form.patient_id) { setError(t("errPatient")); return; }
    const validLines = lines.filter((l) => l.name.trim());
    if (validLines.length === 0) { setError(t("errNoMed")); return; }
    ...
    if (e || !ord) { setError(e?.message ?? t("errGeneric")); setSaving(false); return; }
    ...
    <>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => router.push(`/${locale}/dashboard/medicaments`)} className="px-4 py-2 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-sm font-medium transition-colors">💊 {t("catalog")}</button>
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium transition-colors">+ {t("newOrdonnance")}</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            {!search && <button onClick={openAdd} className="mt-4 px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium">+ {t("newOrdonnance")}</button>}
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 dark:border-zinc-800">
                  ...
                  <th className="px-5 py-3 text-start font-medium text-zinc-500 dark:text-zinc-400">{t("col.prescriber")}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((o) => (
                  ...
                  {lines.map((l, i) => (
                    <div key={i} className="rounded-xl border border-zinc-200 dark:border-zinc-700 p-3 space-y-2 bg-zinc-50/60 dark:bg-zinc-800/30">
                      ...
                      <div className="flex gap-2 items-center">
                        <input placeholder={t("form.medPlaceholder")} value={l.name} onChange={(e) => setLines((xs) => xs.map((x, j) => j === i ? { ...x, name: e.target.value } : x))} className={`flex-1 ${inputCls}`} />
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <input placeholder={t("form.posologie")} value={l.posologie} onChange={(e) => setLines((xs) => xs.map((x, j) => j === i ? { ...x, posologie: e.target.value } : x))} className={inputCls} />
                        <input placeholder={t("form.duree")} value={l.duree} onChange={(e) => setLines((xs) => xs.map((x, j) => j === i ? { ...x, duree: e.target.value } : x))} className={inputCls} />
                        <input placeholder={t("form.quantite")} value={l.quantite} onChange={(e) => setLines((xs) => xs.map((x, j) => j === i ? { ...x, quantite: e.target.value } : x))} className={inputCls} />
                      </div>
                      <input placeholder={t("form.instructions")} value={l.instructions} onChange={(e) => setLines((xs) => xs.map((x, j) => j === i ? { ...x, instructions: e.target.value } : x))} className={inputCls} />
                    </div>
                  ))}
                </div>
                <button type="button" onClick={() => setLines((xs) => [...xs, { ...emptyLine }])} className="mt-2 text-xs text-teal-600 dark:text-teal-400 hover:underline font-medium">+ {t("form.addMed")}</button>
              </div>
