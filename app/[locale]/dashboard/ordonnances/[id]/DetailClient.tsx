...
export default function OrdonnanceDetailClient({ ordonnance: initial, lignes: initialLignes, locale }: Props) {
  ...
  const t = useTranslations("ordonnances");
  ...
  async function saveEdit() {
    ...
    if (valid.length === 0) { setError(t("errNoMed")); return; }
    ...
    if (e || !data) { setError(e?.message ?? t("errGeneric")); setSaving(false); return; }
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">{t("detailTitle")}</h1>
              {annulee && <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">{t("cancelled")}</span>}
            </div>
      ...
      <div className="space-y-6 max-w-3xl">
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6">
          ...
          <div className="space-y-1">
            ...
            <DR label={t("detail.date")} value={fmtDate(ordo.date)} />
            <DR label={t("detail.prescriber")} value={ordo.prescriber ? t("drName", { name: ordo.prescriber }) : null} />
            {visite && (
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6">
          ...
          {lignes.length === 0 ? (
            <p className="text-sm text-zinc-400 py-4 text-center">{t("detail.noMeds")}</p>
          ) : (
                ...
                <div key={l.id} className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 px-4 py-3">
                  ...
                  {(l.posologie || l.duree || l.quantite) && (
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                      {[l.posologie && `${t("form.posologie")} : ${l.posologie}`, l.duree && `${t("form.duree")} : ${l.duree}`, l.quantite && `${t("form.quantite")} : ${l.quantite}`].filter(Boolean).join("   ·   ")}
                    </p>
                  )}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2 pb-8">
          {annulee ? (
            <button onClick={reactivate} className="px-4 py-2 rounded-lg border border-teal-200 dark:border-teal-800 text-teal-600 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-900/20 text-sm font-medium transition-colors">↩ {t("detail.reactivate")}</button>
          ) : (
            <button onClick={() => setCancelOpen(true)} className="px-4 py-2 rounded-lg border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 text-sm font-medium transition-colors">{t("detail.cancelOrdo")}</button>
          )}
          ...
          {!annulee && (
            <div className="ms-auto">
              <button onClick={openEdit} className="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium transition-colors">✏️ {t("detail.edit")}</button>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
              <h2 className="font-semibold text-zinc-900 dark:text-white">{t("detail.editTitle")}</h2>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[72vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">{t("form.date")}</label>
                  <input type="date" value={editForm.date} onChange={(e) => setEditForm((f) => ({ ...f, date: e.target.value }))} className={inputCls} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">{t("form.prescriber")}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-2">{t("form.medications")} <span className="text-red-500">*</span></label>
                <div className="space-y-3">
                  {editLines.map((l, i) => (
                    <div key={i} className="rounded-xl border border-zinc-200 dark:border-zinc-700 p-3 space-y-2 bg-zinc-50/60 dark:bg-zinc-800/30">
                      {medications.length > 0 && (
                        <select value={l.medicament_id ?? ""} onChange={(e) => {
                          ...
                          <option value="">{t("form.chooseFromCatalog")}</option>
                      ...
                      <input placeholder={t("detail.instructionsShort")} value={l.instructions} onChange={(e) => setEditLines((xs) => xs.map((x, j) => j === i ? { ...x, instructions: e.target.value } : x))} className={inputCls} />
                    </div>
                  ))}
                </div>
                <button type="button" onClick={() => setEditLines((xs) => [...xs, { medicament_id: null, name: "", posologie: "", duree: "", quantite: "", instructions: "" }])} className="mt-2 text-xs text-teal-600 dark:text-teal-400 hover:underline font-medium">+ {t("form.addMed")}</button>
              </div>
            ...
            <div className="flex items-center gap-3 px-6 py-4 border-t border-zinc-100 dark:border-zinc-800">
              <div className="ms-auto flex items-center gap-3">
                <button onClick={() => setEditOpen(false)} className="px-4 py-2 rounded-lg text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">{t("form.cancel")}</button>
                <button onClick={saveEdit} disabled={saving} className="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 disabled:opacity-60 text-white text-sm font-medium transition-colors">{saving ? t("form.saving") : t("form.save")}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {cancelOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl p-6">
            <h2 className="font-semibold text-zinc-900 dark:text-white mb-2">{t("detail.cancelDialogTitle")}</h2>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-2">{t("detail.cancelDialogBody")}</p>
            <p className="text-xs text-zinc-400 mb-6">{t("detail.cancelDialogHint")}</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setCancelOpen(false)} className="px-4 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 text-sm text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">{tc("back")}</button>
              <button onClick={cancelOrdo} disabled={busy} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white text-sm font-medium transition-colors">{busy ? "…" : t("detail.cancelOrdo")}</button>
            </div>
          </div>
        </div>
      )}
