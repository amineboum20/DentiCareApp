  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Praticiens</h1>
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium transition-colors">+ Nouveau praticien</button>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        {praticiens.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Aucun praticien</p>
            <button onClick={openAdd} className="mt-4 px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium">+ Nouveau praticien</button>
          </div>
        ) : (
            ...
            {praticiens.map((p) => (
              <div key={p.id} onClick={() => openEdit(p)} className="flex items-center justify-between px-5 py-3.5 hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors cursor-pointer">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-zinc-900 dark:text-white">Dr. {p.name}{p.speciality ? <span className="text-zinc-400 font-normal"> · {p.speciality}</span> : null}</p>
                  <p className="text-xs text-zinc-400 mt-0.5">{[p.inpe && `INPE ${p.inpe}`, p.numero_ordre && `N° ordre ${p.numero_ordre}`].filter(Boolean).join(" · ") || "—"}</p>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
              <h2 className="font-semibold text-zinc-900 dark:text-white">{editing ? "Modifier le praticien" : "Nouveau praticien"}</h2>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Nom du dentiste" className={inputCls} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">N° d&apos;ordre</label>
                  <input value={form.numero_ordre} onChange={(e) => setForm((f) => ({ ...f, numero_ordre: e.target.value }))} className={inputCls} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">Spécialité</label>
                  <input value={form.speciality} onChange={(e) => setForm((f) => ({ ...f, speciality: e.target.value }))} placeholder="Ex. Orthodontie" className={inputCls} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">Téléphone</label>
