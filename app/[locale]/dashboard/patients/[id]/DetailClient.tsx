    ...
    <>
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400">{t("detail.addedOn", { date: fmtDate(patient.created_at) })}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start max-w-6xl">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 px-6 py-5">
          <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wide mb-3">{t("detail.informations")}</p>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 px-6 py-5">
          <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wide mb-2.5">{t("detail.quickView")}</p>
          {snapshotLoading ? (
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between rounded-lg bg-zinc-50 dark:bg-zinc-800/60 px-3 py-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-sm shrink-0">📅</span>
                  <div className="min-w-0">
                    <p className="text-xs font-medium text-zinc-700 dark:text-zinc-300">{t("detail.nextRdv")}</p>
                    <p className="text-[11px] text-zinc-400">
                ...
                <div className="rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/30 px-3 py-2 space-y-1.5">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <p className="text-xs font-medium text-amber-800 dark:text-amber-300">
                      {t("detail.unpaidFactures", { count: snapshot.activeFactures.length })}
                    </p>
                  </div>
                    ...
                    <div key={f.id} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] text-amber-700 dark:text-amber-400 font-mono">
                          {(f.total_price - f.deposit_paid).toFixed(2)} MAD {t("detail.due")}
                        </span>
                        <button
            ...
            <div className="space-y-8">
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => router.push(`/${locale}/dashboard/dossiers?new=1&patient_id=${patient.id}`)}
                    className="text-xs text-teal-600 dark:text-teal-400 hover:underline font-medium">+ {t("detail.newShort")}</button>
                </div>
                {historyDossiers.length === 0 ? (
                  <p className="text-sm text-zinc-400 py-4 text-center">{t("detail.noDossier")}</p>
                ) : (
                    ...
                    {historyDossiers.map(d => (
                      <div key={d.id} onClick={() => router.push(`/${locale}/dashboard/dossiers/${d.id}`)}
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ms-2 ${d.statut === "termine" ? "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400" : "bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300"}`}>
                          {d.statut === "termine" ? tst("termine") : tst("ouvert")}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                ...
                {historyConsultations.length === 0 ? (
                  <p className="text-sm text-zinc-400 py-4 text-center">{t("detail.noVisite")}</p>
                ) : (
                  <div className="space-y-3">
                    {historyConsultations.map(c => (
                      <div key={c.id} onClick={() => router.push(`/${locale}/dashboard/consultations/${c.id}`)}
                        ...
                        {c.next_exam_date && <p className="text-xs text-zinc-400 mb-1">{t("detail.nextLine", { date: fmtDate(c.next_exam_date) })}</p>}
                        {c.treated_by && <p className="text-xs text-zinc-400 mb-1">{t("detail.drName", { name: c.treated_by })}</p>}
              ...
              <div>
                ...
                {historyFactures.length === 0 ? (
                  <p className="text-sm text-zinc-400 py-4 text-center">{t("detail.noFacture")}</p>
                ) : (
                  <div className="space-y-2">
                      ...
                      <div key={f.id} onClick={() => router.push(`/${locale}/dashboard/factures/${f.id}`)}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-right">
                            ...
                            {f.deposit_paid > 0 && <p className="text-xs text-zinc-400">{t("detail.acShort")}: {f.deposit_paid.toFixed(2)} MAD</p>}
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl p-6">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
              {t("archive.subtitle", { name: `${patient.first_name} ${patient.last_name}` })}
            </p>
            {archivePreview && (
