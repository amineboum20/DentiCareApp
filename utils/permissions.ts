 ...
 *                     catalog, reports, suppliers or Paramètres (the cabinet
 *                     view). Everyone has "Mon profil" (name, email, password,
 *                     language).
 *
...
export function canAccessPath(role: MemberRole, path: string): boolean {
  ...
  if (path === "/dashboard/profile") return true; // own name, email, password, language
