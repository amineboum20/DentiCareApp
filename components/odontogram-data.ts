// Anatomical tooth SVG paths for the odontogram.
// Upper-right quadrant (tooth 1 = central incisor … 8 = third molar); the four
// QUADRANTS transforms mirror this set into a full mouth (viewBox 409 x 694).
//
// Tooth paths adapted from react-odontogram by biomathcode — MIT License.
// https://github.com/biomathcode/react-odontogram

export interface ToothPath {
  name: string;
  type: string;
  outlinePath: string;
  shadowPath: string;
...
export const teethPaths: ToothPath[] = [
	{
		name: "1",
...
export const QUADRANTS: { transform: string }[] = [
  { transform: "" },                                    // Q1 upper-right
  { transform: "scale(-1, 1) translate(-409, 0)" },     // Q2 upper-left
  { transform: "scale(1, -1) translate(0, -694)" },     // Q3 lower-right
  { transform: "scale(-1, -1) translate(-409, -694)" }, // Q4 lower-left
];

// FDI quadrant digit per position: adults 1-4, deciduous (children) 5-8.
export const ADULT_PREFIX = ["1", "2", "3", "4"];
export const CHILD_PREFIX = ["5", "6", "7", "8"];

// Centre of each tooth (1..8) in Q1 coords — for placing numbers un-mirrored.
export const ANCHORS: Record<string, { x: number; y: number }> = {
  "1": { x: 179, y: 31 }, "2": { x: 133, y: 36 }, "3": { x: 101, y: 57 }, "4": { x: 77, y: 87 },
  "5": { x: 59, y: 121 }, "6": { x: 43, y: 171 }, "7": { x: 35, y: 234 }, "8": { x: 32, y: 290 },
};

export const VIEWBOX = { w: 409, h: 694 };

// Below age 13 a patient is charted on the deciduous (child) odontogram.
export const CHILD_MAX_AGE = 13;
export function isChildAge(dob: string | null | undefined): boolean {
  if (!dob) return false; // no birthdate → adult chart
...
export const STATUS_KEYS = ["carie", "obturee", "couronne", "a_traiter", "prothese", "bridge", "implant", "absente"] as const;
export const STATUS_COLORS: Record<string, string> = {
  carie: "#dc2626", obturee: "#2563eb", couronne: "#d97706", a_traiter: "#ea580c",
...
export function buildOdontogramSvg(chart: Record<string, { status: string }>, isChild: boolean): string {
  ...
  QUADRANTS.forEach((q, qi) => {
    g += `<g${q.transform ? ` transform="${q.transform}"` : ""}>`;
    for (const tp of teeth) {
  ...
  QUADRANTS.forEach((_, qi) => {
    for (const tp of teeth) {
      const fdi = prefixes[qi] + tp.name;
      const p = anchorAt(qi, ANCHORS[tp.name]);
      const col = chart[fdi]?.status ? STATUS_COLORS[chart[fdi].status] : "#3f3f46";
      g += `<text x="${p.x}" y="${p.y}" text-anchor="middle" dominant-baseline="middle" font-size="17" font-weight="700" fill="${col}" font-family="Helvetica,Arial,sans-serif">${fdi}</text>`;
    }
  });
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${VIEWBOX.w} ${VIEWBOX.h}" fill="none">${g}</svg>`;
}
